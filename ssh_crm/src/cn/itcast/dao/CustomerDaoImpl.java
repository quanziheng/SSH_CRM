...
//		//创建一个list集合，如果值不为空，把值设置到list中
//		List<Object> p = new ArrayList<Object>();
//		
//		//判断条件值是否为空，如果不为空则拼接hql语句
...
//		}		
//		return (List<Customer>) this.getHibernateTemplate().find(hql, p.toArray());//第二个参数是Object可变参数，视为数组;
		//创建离线对象,指定对哪个实体类进行操作
		DetachedCriteria criteria = DetachedCriteria.forClass(Customer.class);
		//判断条件值是否为空
		if(customer.getCustName()!=null && !"".equals(customer.getCustName())){
			//设置对属性、设置值
			criteria.add(Restrictions.eqOrIsNull("custName", customer.getCustName()));
		}
//		if(customer.getCustLevel()!=null && !"".equals(customer.getCustLevel())){
